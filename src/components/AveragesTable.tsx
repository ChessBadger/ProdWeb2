...
interface PerformanceByGroupTableProps {
  data: EmployeeRecord[];
  groupBy: "store" | "supervisor" | "account";
}
...
export const PerformanceByGroupTable: React.FC<
  PerformanceByGroupTableProps
> = ({ data, groupBy }) => {
  ...
  if (groupBy === "store") headerLabel = "Store";
  else if (groupBy === "supervisor") headerLabel = "Supervisor";
      ...
      <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
        <thead className="bg-slate-50 dark:bg-slate-700/50">
          <tr>
            ...
            {METRIC_OPTIONS.map((m) =>
              renderSortableHeader(m.value, `Avg ${m.label}`, "text-center")
            )}
          </tr>
        </thead>
        <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
          {sortedData.map((row) => (
            ...
            >
              <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-slate-800 dark:text-slate-200 sticky left-0 bg-white dark:bg-slate-800 group-hover:bg-slate-50 dark:group-hover:bg-slate-700/50">
                {row.groupName}
              </td>
              {METRIC_OPTIONS.map((m) => (
                <td
                  key={`avg-${m.value}`}
                  className="px-4 py-3 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400 text-center"
                >
                  {row.averages[m.value].toFixed(2)}
                </td>
              ))}
            </tr>
          ))}
            ...
            {METRIC_OPTIONS.map((m) => (
              <div key={`overall-${m.value}`}>
                ...
                <div className="text-xl font-bold text-primary">
                  {overallAverages[m.value].toFixed(2)}
                </div>
              </div>
            ))}
            <div>
              ...
              <div className="text-xl font-bold text-primary">
                {overallConsistencyScore.toFixed(1)}
              </div>
            </div>
          </div>
        </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
          <thead className="bg-slate-50 dark:bg-slate-700/50">
            <tr>
              ...
              {METRIC_OPTIONS.map((m) =>
                renderHeader(m.value, `Avg ${m.label}`)
              )}
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
            {sortedData.map((row) => (
              ...
              >
                ...
                {METRIC_OPTIONS.map((m) => (
                  <td
                    key={m.value}
                    className="px-4 py-3 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"
                  >
                    {row[m.value].toFixed(2)}
                  </td>
                ))}
              </tr>
            ))}
