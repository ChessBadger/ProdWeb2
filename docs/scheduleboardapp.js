const state = {
  ...
  residualGlobal: { count: 0, mean: 0, p10: 0, p25: 0, p75: 0, p90: 0, stdDev: 0 },
  ...
  residualProfilesByDensity: {
    dense: {
      ks: { account: 24, segment: 14, type: 16, office: 16, store: 8, accountSupervisor: 10, storeSupervisor: 6, globalBand: 18, accountBand: 14, segmentBand: 12, typeBand: 12, storeBand: 8 },
      floors: { minStoreWeight: 0.3, anchorMinShare: 0.4, anchorMinWeight: 0.35, anchorK: 8, anchorMaxAbsHours: 0.7 },
      mins: { storeBand: 5, storeSupervisor: 6, store: 8, accountSupervisor: 10, segmentBand: 10, segment: 16, typeBand: 12, type: 18, office: 16, accountBand: 16, account: 24, globalBand: 20 },
    },
    sparse: {
      ks: { account: 12, segment: 8, type: 8, office: 8, store: 5, accountSupervisor: 6, storeSupervisor: 4, globalBand: 12, accountBand: 8, segmentBand: 8, typeBand: 8, storeBand: 5 },
      floors: { minStoreWeight: 0.45, anchorMinShare: 0.55, anchorMinWeight: 0.45, anchorK: 6, anchorMaxAbsHours: 0.9 },
      mins: { storeBand: 3, storeSupervisor: 4, store: 5, accountSupervisor: 6, segmentBand: 6, segment: 8, typeBand: 8, type: 10, office: 8, accountBand: 8, account: 12, globalBand: 12 },
    },
  },
...
function bootstrapAuth() {
  if (typeof firebase === "undefined") return showAuthOverlay("Authentication is unavailable right now. Please refresh.");
  if (!firebase.apps.length) firebase.initializeApp(firebaseConfig);
  auth = firebase.auth();
  googleProvider = new firebase.auth.GoogleAuthProvider();
  dom.signOutBtn?.addEventListener("click", () => auth?.signOut().catch(() => {}));
...
async function loadData() {
  try {
    ...
    setBoardMeta(`Data load failed: ${error?.message || "Unknown error"}`, true);
  } finally {
    hideLoadingOverlay();
  }
}
...
function extractRows(payload) {
  ...
  const first = Object.values(payload || {}).find((value) => Array.isArray(value));
  return first || [];
}

function buildHistory(rows, rawJsonText = "") {
  const normalizedRows = rows.map(normalizeHistoryRow).filter((row) => row.valid);
...
function normalizeHistoryRow(row) {
  ...
  const account = clean(firstValue(normalized, ["account", "accountname"])) || "Unknown Account";
  ...
  const type = normalizeInventoryType(clean(firstValue(normalized, ["typeofinv"])) || "Unknown");
  const officeName = clean(firstValue(normalized, ["officename"])) || "Unknown";
  const role = clean(firstValue(normalized, ["role", "employeerole", "position", "jobtitle"])).toLowerCase();
  ...
  return { valid: Boolean(date && store && employee), date, account, store, storeKey, employee, employeeName, type, officeName, role, supervisorNumber, manHours, totalExtQty, piecesPerHr, jobKey };
}
...
function buildJobs(rows) {
  ...
  rows.forEach((row) => {
    if (!jobs.has(row.jobKey)) {
      jobs.set(row.jobKey, { date: row.date, account: row.account, storeName: row.store, storeKey: row.storeKey, typeOfInv: row.type, officeName: row.officeName || "Unknown", supervisorNumber: clean(row.supervisorNumber), supervisorCounts: new Map(), employees: new Set(), totalPieces: 0, totalManHours: 0, duration: 0 });
    }
    const job = jobs.get(row.jobKey);
    const supervisorId = clean(row.supervisorNumber);
    if (supervisorId) {
      job.supervisorCounts.set(supervisorId, (job.supervisorCounts.get(supervisorId) || 0) + 1);
      if (!job.supervisorNumber) job.supervisorNumber = supervisorId;
    }
    if (row.officeName && row.officeName !== "Unknown") job.officeName = row.officeName;
  ...
  return Array.from(jobs.values()).map((job) => ({ ...job, crewSize: job.employees.size, employees: Array.from(job.employees), supervisorNumber: resolveMostFrequentKey(job.supervisorCounts, job.supervisorNumber), supervisorCounts: undefined }));
}

function buildStoreStats(jobs) {
  const grouped = new Map();
  jobs.forEach((job) => {
    if (!grouped.has(job.storeKey)) grouped.set(job.storeKey, { storeKey: job.storeKey, account: job.account, storeName: job.storeName, jobs: [], typeCounts: new Map() });
    const bucket = grouped.get(job.storeKey);
    bucket.jobs.push(job);
    bucket.typeCounts.set(job.typeOfInv, (bucket.typeCounts.get(job.typeOfInv) || 0) + 1);
  });
  const stores = new Map();
  grouped.forEach((bucket, storeKey) => {
    const primaryType = Array.from(bucket.typeCounts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] || "Unknown";
    const summary = summarizeJobGroup(bucket.jobs);
    stores.set(storeKey, { storeKey, account: bucket.account, storeName: bucket.storeName, officeName: bucket.jobs[0]?.officeName || "Unknown", avgPieces: summary.avgPieces, medianPieces: summary.medianPieces, trimmedMeanPieces: summary.trimmedMeanPieces, recentWeightedPieces: summary.recentWeightedPieces, avgDuration: summary.avgDuration, medianDuration: summary.medianDuration, avgManHours: summary.avgManHours, durationStdDev: summary.durationStdDev, robustDurationSpread: summary.robustDurationSpread, overheadBaseP20: summary.overheadBaseP20, jobCount: bucket.jobs.length, primaryType });
    });
  return stores;
}
...
function buildStoreSegments(stores) {
  ...
  stores.forEach((store, storeKey) => {
    ...
    byAccount.get(store.account).push({ storeKey, sizeSignal: getStoreSizeSignal(store) });
  });
  ...
  byAccount.forEach((entries, account) => {
    ...
    sorted.forEach((item, idx) => {
      const bucket = Math.min(segmentCount - 1, Math.floor((idx * segmentCount) / Math.max(1, sorted.length)));
...
function applySegmentsToStores(stores, storeSegmentByStoreKey) {
  stores.forEach((store, storeKey) => {
    const segment = storeSegmentByStoreKey.get(storeKey) || { segmentId: "S1", segmentKey: `${store.account}||S1` };
    store.segmentId = segment.segmentId;
    store.segmentKey = segment.segmentKey;
  });
}

function buildAccountSegmentStats(jobs) { return buildScopedJobStats(jobs, (job) => state.storeSegmentByStoreKey.get(job.storeKey)?.segmentKey || ""); }
function buildAccountTypeStats(jobs) { return buildScopedJobStats(jobs, (job) => `${job.account}||${job.typeOfInv || "Unknown"}`); }
function buildAccountOfficeStats(jobs) { return buildScopedJobStats(jobs, (job) => `${job.account}||${job.officeName || "Unknown"}`); }
function buildAccountGlobalStats(jobs) { return buildScopedJobStats(jobs, (job) => job.account || ""); }
...
function buildScopedJobStats(jobs, keyFn) {
  ...
  grouped.forEach((jobGroup, key) => {
    ...
    stats.set(key, { avgPieces: summary.avgPieces, medianPieces: summary.medianPieces, trimmedMeanPieces: summary.trimmedMeanPieces, recentWeightedPieces: summary.recentWeightedPieces, avgDuration: summary.avgDuration, robustDurationSpread: summary.robustDurationSpread, overheadBaseP20: summary.overheadBaseP20, jobCount: summary.jobCount });
  });
  return stats;
}
...
function buildEmployeeStats(rows) {
  const grouped = new Map();
  const maxRowTimestamp = (rows || []).map((row) => Date.parse(row.date || "")).filter((ts) => Number.isFinite(ts)).reduce((max, ts) => Math.max(max, ts), 0);
  const referenceTimestamp = maxRowTimestamp > 0 ? maxRowTimestamp : Date.now();
  rows.forEach((row) => {
    const isSupervisorRunRow = clean(row.employee).toLowerCase() !== "" && clean(row.supervisorNumber).toLowerCase() !== "" && clean(row.employee).toLowerCase() === clean(row.supervisorNumber).toLowerCase();
    if (isSupervisorRunRow) return;
    if (!grouped.has(row.employee)) grouped.set(row.employee, { employee: row.employee, weightedSpeedSumGlobal: 0, weightSumGlobal: 0, recentWeightedSpeedSumGlobal: 0, recentWeightSumGlobal: 0, jobKeysGlobal: new Set(), accountBuckets: new Map(), nameCounts: new Map(), officeCounts: new Map() });
    const speed = inferSpeed(row);
    const weight = inferWeight(row);
    const rowTimestamp = Date.parse(row.date || "");
    const ageDays = Number.isFinite(rowTimestamp) ? Math.max(0, (referenceTimestamp - rowTimestamp) / 86400000) : 0;
    ...
    if (!bucket.accountBuckets.has(row.account)) bucket.accountBuckets.set(row.account, { weightedSpeedSum: 0, weightSum: 0, recentWeightedSpeedSum: 0, recentWeightSum: 0, jobKeys: new Set() });
    ...
    if (row.employeeName) bucket.nameCounts.set(row.employeeName, (bucket.nameCounts.get(row.employeeName) || 0) + 1);
    if (row.officeName) bucket.officeCounts.set(row.officeName, (bucket.officeCounts.get(row.officeName) || 0) + 1);
  });
  const stats = new Map();
  grouped.forEach((bucket, employee) => {
    ...
    bucket.accountBuckets.forEach((accountBucket, account) => {
      accountStats[account] = { avgPiecesPerHr: accountBucket.weightSum > 0 ? accountBucket.weightedSpeedSum / accountBucket.weightSum : 0, avgPiecesPerHrRecent: accountBucket.recentWeightSum > 0 ? accountBucket.recentWeightedSpeedSum / accountBucket.recentWeightSum : 0, jobCount: accountBucket.jobKeys.size };
    });
    stats.set(employee, { employee, displayName, officeName: mostFrequent(bucket.officeCounts) || "Unknown", avgPiecesPerHrGlobal: bucket.weightSumGlobal > 0 ? bucket.weightedSpeedSumGlobal / bucket.weightSumGlobal : 0, avgPiecesPerHrRecentGlobal: bucket.recentWeightSumGlobal > 0 ? bucket.recentWeightedSpeedSumGlobal / bucket.recentWeightSumGlobal : 0, globalJobCount: bucket.jobKeysGlobal.size, accountStats });
  });
  return stats;
}
...
function buildGlobalStats(jobs, employees) {
  ...
  const speeds = Array.from(employees.values()).map((employee) => employee.avgPiecesPerHrGlobal).filter((value) => value > 0);
  return { avgPieces: summary.avgPieces, medianPieces: summary.medianPieces, trimmedMeanPieces: summary.trimmedMeanPieces, recentWeightedPieces: summary.recentWeightedPieces, durationStdDev: summary.durationStdDev, robustDurationSpread: summary.robustDurationSpread, overheadBaseP20: summary.overheadBaseP20, medianEmployeeSpeed: median(speeds) };
}
...
function buildStoreLastSupervisor(rows) {
  ...
  (rows || []).forEach((row) => {
    ...
    if (!prev || currentDate > prev.date) {
      latestByStore.set(storeKey, { date: currentDate, counts: new Map([[supervisor, 1]]) });
      return;
    }
    if (currentDate === prev.date) prev.counts.set(supervisor, (prev.counts.get(supervisor) || 0) + 1);
  });
...
function buildRuns(rows) {
  ...
  const preparedStops = rows.map((row, sourceIndex) => {
    ...
    const storeKey = matchedKeys
      .map((key) => state.stores.get(key))
      .sort((a, b) => (b?.jobCount || 0) - (a?.jobCount || 0))[0]?.storeKey || "";
    const store = state.stores.get(storeKey);
    const runName = clean(row.RunName);
    const hints = parseHints(runName);
    return {
      id: [date, canon(storeName), clean(row.CustomerNumber1), sourceIndex].join("||"),
      date,
      runName,
  ...
  const grouped = new Map();
  preparedStops.forEach((stop) => {
    const groupKey = groupedRunKeys.has(`${stop.date}||${stop.runName}`) ? `${stop.date}||${stop.runName}` : stop.id;
    if (!grouped.has(groupKey)) grouped.set(groupKey, []);
    grouped.get(groupKey).push(stop);
  });

  state.runs = Array.from(grouped.values()).map((stops) => {
    const orderedStops = [...stops].sort((a, b) => {
      const byOrder = runOrderSortValue(a.runOrderNumber) - runOrderSortValue(b.runOrderNumber);
      if (byOrder !== 0) return byOrder;
      const byTime = timeMinutes(a.startTime) - timeMinutes(b.startTime);
      if (byTime !== 0) return byTime;
      return a.sourceIndex - b.sourceIndex || a.storeName.localeCompare(b.storeName);
    });
    const primary = orderedStops[0];
    const uniqueAccounts = Array.from(new Set(orderedStops.map((stop) => stop.account).filter(Boolean)));
    const uniqueOffices = Array.from(new Set(orderedStops.map((stop) => stop.officeLabel).filter(Boolean)));
    const uniqueTypes = Array.from(new Set(orderedStops.map((stop) => stop.typeOfInv).filter(Boolean)));
    const matchCount = orderedStops.filter((stop) => stop.historyMatched).length;
    const historyInventoryCount = orderedStops.reduce((sum, stop) => sum + Math.max(0, safeNumber(stop.historyInventoryCount)), 0);
    return {
      id: orderedStops.length > 1 ? `group||${primary.date}||${canon(primary.runName)}` : primary.id,
      date: primary.date,
      runName: primary.runName,
      runOrderNumber: primary.runOrderNumber,
      sourceIndex: primary.sourceIndex,
      storeName: primary.storeName,
      account: uniqueAccounts.length === 1 ? uniqueAccounts[0] : `${uniqueAccounts.length} accounts`,
      primaryAccount: uniqueAccounts.find((account) => account && account !== "Unmatched Store") || uniqueAccounts[0] || "",
      ...
      stops: orderedStops,
      meetTime: firstScheduledValue(orderedStops.map((stop) => stop.meetTime)),
      startTime: firstScheduledValue(orderedStops.map((stop) => stop.startTime).sort((left, right) => timeMinutes(left) - timeMinutes(right))),
      typeOfInv: uniqueTypes.length === 1 ? uniqueTypes[0] : uniqueTypes.length ? "Mixed" : "",
      ...
      officeLabel: uniqueOffices.length === 1 ? uniqueOffices[0] : uniqueOffices.length ? "Multiple" : "Unknown",
  ...
  }).sort((a, b) => a.date.localeCompare(b.date) || timeMinutes(a.startTime) - timeMinutes(b.startTime) || a.sortOrder - b.sortOrder || a.sourceIndex - b.sourceIndex || displayRunTitle(a).localeCompare(displayRunTitle(b)));
}
...
function buildWeeks() {
  ...
  state.weeks = Array.from(map.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([key, runs]) => ({
    key,
...
function pickDefaultWeekKey() {
  ...
  return state.weeks.find((week) => week.dates.includes(today))?.key || state.weeks[0]?.key || "";
}
...
function getVisibleRuns() {
  ...
  return runs.filter((run) => {
    const haystack = [
      ...
      run.storeNotes,
      run.address,
      ...run.stops.map((stop) => `${stop.storeName} ${stop.account} ${stop.customerNumber} ${stop.notes} ${stop.storeNotes} ${stop.address}`),
    ].join(" ").toLowerCase();
...
function toggleDetailPanel(forceValue = null) {
  state.detailCollapsed = typeof forceValue === "boolean" ? forceValue : !state.detailCollapsed;
  renderBoardLayoutState();
}
...
function renderBoardLayoutState() {
  ...
  if (dom.toggleDetailBtn) dom.toggleDetailBtn.textContent = state.detailCollapsed ? "Show Details" : "Hide Details";
  if (dom.collapseDetailBtn) dom.collapseDetailBtn.textContent = state.detailCollapsed ? "Show Panel" : "Hide Panel";
...
function renderBoard() {
  const week = state.weeks.find((item) => item.key === state.selectedWeekKey);
  if (!week) {
    dom.boardGrid.innerHTML = '<div class="empty-state">No scheduled runs available.</div>';
    dom.boardTopScroll?.classList.add("is-hidden");
    return;
  }
  if (state.focusedDate && !week.dates.includes(state.focusedDate)) state.focusedDate = "";
  ...
  if (!datesToRender.length) {
    dom.boardGrid.innerHTML = '<div class="empty-state">No run days match the current filters.</div>';
    dom.boardTopScroll?.classList.add("is-hidden");
    return;
  }
  dom.boardGrid.innerHTML = datesToRender.map((date) => {
  ...
  dom.boardGrid.querySelectorAll(".day-column[data-day-date]").forEach((column) => {
    column.addEventListener("click", (event) => {
      if (event.target.closest("[data-run-id]") || event.target.closest("[data-focus-date]")) return;
...
function renderRunStops(run) {
  return `<div class="run-stops">${run.stops.map((stop, index) => {
    const startLabel = stop.startTime || "Start TBD";
    const address = stop.mapLink ? `<a class="run-link" href="${escapeHtml(stop.mapLink)}" target="_blank" rel="noopener noreferrer">${escapeHtml(stop.address || "Open map")}</a>` : escapeHtml(stop.address || "Not listed");
...
function renderDetailPanel() {
  ...
  if (!run) {
    ...
    dom.supervisorSelect.innerHTML = '<option value="">Select supervisor</option>';
  ...
  dom.predictedDurationValue.textContent = prediction ? `${fmt(prediction.onSiteDuration, 2)} hrs` : "-";
  dom.predictedManHoursValue.textContent = prediction ? `${fmt(prediction.manHours, 2)} man-hours` : "-";
  dom.predictedDeltaValue.textContent = prediction?.deltaAvailable ? `${signed(prediction.deltaHours, 2)} hrs` : "No goal";
  dom.assignedSummary.textContent = buildAssignmentSummary(run, assignment, prediction);
  renderEmployeeGroups();
}
...
function renderEmployeeGroups() {
  ...
  dom.employeeGroups.innerHTML = groups.length ? groups.map(([office, employees]) => `<section class="employee-group"><div class="employee-group-head">${escapeHtml(office)} (${employees.length})</div><div class="employee-list">${employees.map((employee) => row(employee, assignment)).join("")}</div></section>`).join("") : '<div class="empty-state">No employees match this filter.</div>';
  dom.employeeGroups.querySelectorAll("[data-employee-id]").forEach((checkbox) => {
    checkbox.addEventListener("change", () => {
      ...
      a.employeeIds = Array.from(ids).sort((aId, bId) => nameOf(aId).localeCompare(nameOf(bId)));
      syncRoleAssignmentsToCrew(run.id);
      persistAssignments();
      renderBoard();
      renderDetailPanel();
...
function applyBulkEmployeeSelection(run, bulk) {
  ...
  const assignedSupervisor = assignFirstBulkSupervisorForRun(run.id, bulk.matchedIds);
  ...
  setEmployeeBulkStatus(
    ...
    bulk.ambiguousEntries.length || bulk.unmatchedEntries.length ? "warning" : "success",
  );
}
...
function addEmployeeToRun(runId, employeeId) {
  ...
  if (!assignment.employeeIds.includes(employeeId)) {
    assignment.employeeIds = [...assignment.employeeIds, employeeId].sort((aId, bId) => nameOf(aId).localeCompare(nameOf(bId)));
...
function setSupervisorForRun(runId, employeeId) {
  ...
  assignment.employeeIds = assignment.employeeIds.filter((id) => id !== assignment.supervisorId);
  syncRoleAssignmentsToCrew(runId);
}
...
function applyBulkRoleHintsToRun(runId, bulk) {
  ...
  assignment.rxIds = filterToAssigned([...assignment.rxIds, ...(bulk?.rxIds || [])], assignedSet);
  assignment.earlyLateIds = filterToAssigned([...assignment.earlyLateIds, ...(bulk?.earlyLateIds || [])], assignedSet);
}
...
function renderRoleSelectors(run) {
  ...
  dom.supervisorSelect.innerHTML = '<option value="">Select supervisor</option>' + sortedSelected
    .map((employee) => `<option value="${escapeHtml(employee.id)}" ${employee.id === assignment.supervisorId ? "selected" : ""}>${escapeHtml(employee.name)}</option>`)
    .join("");

  if (dom.supervisorMode) dom.supervisorMode.value = assignment.roleModes.supervisor;
  dom.rxRoleCard?.classList.toggle("is-hidden", !showRxRole);
  renderRoleChecklist(dom.rxEmployee, sortedSelected, assignment.rxIds, "No RX role");
  renderRoleChecklist(dom.trainingEmployee, sortedSelected, assignment.trainingIds, "No training role");
  renderRoleChecklist(dom.earlyLateEmployee, sortedSelected, assignment.earlyLateIds, "No early/late role");
  ...
  if (dom.earlyLateMode) dom.earlyLateMode.value = assignment.roleModes.earlyLate;
}
...
function syncRoleAssignmentsToCrew(runId) {
  ...
  assignment.rxIds = runNeedsRxRole(run) ? filterToAssigned(assignment.rxIds, assignedSet) : [];
  assignment.trainingIds = filterToAssigned(assignment.trainingIds, assignedSet);
  assignment.earlyLateIds = filterToAssigned(assignment.earlyLateIds, assignedSet);
}

function getContributionFactorForAssignedEmployee(employeeId, assignment) {
  let factor = 1;
  if (assignment.supervisorId === employeeId) factor = Math.min(factor, getContributionFactor(assignment.roleModes.supervisor));
  if (assignment.rxIds.includes(employeeId)) factor = Math.min(factor, getContributionFactor(assignment.roleModes.rx));
  if (assignment.trainingIds.includes(employeeId)) factor = Math.min(factor, getContributionFactor(assignment.roleModes.training));
  if (assignment.earlyLateIds.includes(employeeId)) factor = Math.min(factor, getContributionFactor(assignment.roleModes.earlyLate));
  return factor;
}
...
function renderCardCrew(run, assignment) {
  const supervisor = assignment.supervisorId ? nameOf(assignment.supervisorId) : "Unassigned";
  const grouped = new Map();
  ...
  const groups = Array.from(grouped.entries())
    ...
    .map(([office, names]) => `<div class="card-crew-group"><div class="card-crew-office">${escapeHtml(office)}</div><div class="card-crew-names">${escapeHtml(names.sort((a, b) => a.localeCompare(b)).join(", "))}</div></div>`)
    .join("");
...
function predictRun(run) {
  ...
  const stopPredictions = run.stops.map((stop) => predictForAssignedCrewExact(stop.storeKey, ids, roles, modes)).filter(Boolean);
  if (stopPredictions.length !== run.stops.length) return null;
  const onSiteDuration = stopPredictions.reduce((sum, prediction) => sum + prediction.onSiteDuration, 0);
  const manHours = stopPredictions.reduce((sum, prediction) => sum + prediction.manHours, 0);
  return {
    onSiteDuration,
    manHours,
    deltaAvailable: run.plannedDurationHours > 0,
    deltaHours: run.plannedDurationHours > 0 ? onSiteDuration - run.plannedDurationHours : 0,
  };
}

function buildAssignmentSummary(run, assignment, prediction) {
  const ids = assignedIds(run.id);
  const officeSummary = Array.from(ids.reduce((map, id) => {
    const office = state.employees.get(id)?.officeName || "Unknown";
    map.set(office, (map.get(office) || 0) + 1);
    return map;
  }, new Map()).entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([office, count]) => `${office} ${count}`).join(", ");
  const effectiveRxIds = runNeedsRxRole(run) ? assignment.rxIds : [];
  const roleSummary = [
    effectiveRxIds.length ? `RX ${effectiveRxIds.length}` : "",
    assignment.trainingIds.length ? `Training ${assignment.trainingIds.length}` : "",
    assignment.earlyLateIds.length ? `Early/Late ${assignment.earlyLateIds.length}` : "",
  ].filter(Boolean).join(", ");
  return [
    ...
    prediction ? `Prediction ready.` : "Assign a crew to generate a prediction.",
  ].filter(Boolean).join(" ");
}
...
function getAssignment(runId) {
  if (!state.assignments[runId]) state.assignments[runId] = createEmptyAssignment();
...
function assignedIds(runId) {
  ...
  return Array.from(new Set([a.supervisorId, ...a.employeeIds].filter(Boolean)));
}

function restoreAssignments() {
  try {
    state.assignments = JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}") || {};
  } catch (_error) {
    state.assignments = {};
  }
}
...
function sortedEmployees() {
  return Array.from(state.employees.values()).sort((a, b) => a.officeName.localeCompare(b.officeName) || a.displayName.localeCompare(b.displayName));
}

function filteredEmployees() {
  return !state.employeeFilter
    ? sortedEmployees()
    : sortedEmployees().filter((employee) => matchesEmployeeQuery(employee.employee, state.employeeFilter, `${employee.displayName} ${employee.officeName}`));
}

function displayRunTitle(run) {
  if (!run?.isGrouped) return run?.storeName || "Store";
  const label = clean(run.runName).replace(/^\d+/, "").trim();
  if (!label || /^length of store\b/i.test(label) || /^crew size of\b/i.test(label)) {
...
function buildPlannerHref(run, assignment) {
  ...
  const payload = {
    ...
    roleModes: {
      supervisor: parseContributionMode(assignment?.roleModes?.supervisor || "p50"),
      rx: parseContributionMode(assignment?.roleModes?.rx || "p50"),
      training: parseContributionMode(assignment?.roleModes?.training || "p70"),
      earlyLate: parseContributionMode(assignment?.roleModes?.earlyLate || "p50"),
    },
    sourceRun: {
  ...
  return encoded ? `scheduleplanning.html?boardPrefill=${encodeURIComponent(encoded)}` : "";
}
...
function parseContributionMode(value) {
  if (value === "full" || value === "none" || value === "p70" || value === "p50" || value === "p30") return value;
  return "p50";
}
...
function displayEmployeeSpeed(employee, account = "") {
  ...
  if (accountStat && accountStat.jobCount >= 1) {
    const blended = blendRecentAndLongSpeed(accountStat.avgPiecesPerHrRecent, accountStat.avgPiecesPerHr, accountStat.jobCount);
    return shrinkTowardFallback(blended, fallback, accountStat.jobCount, 3);
  }
  if (employee.globalJobCount >= 1) {
    const blended = blendRecentAndLongSpeed(employee.avgPiecesPerHrRecentGlobal, employee.avgPiecesPerHrGlobal, employee.globalJobCount);
...
function predictForAssignedCrewExact(storeKey, crewIds, rolesConfig, modesConfig) {
  const store = state.stores.get(storeKey);
  const roles = {
  ...
  if (isRxRoleRequiredForStore(store) && !roles.rx.some((id) => crew.includes(id))) return null;
  ...
  const baseline = resolveBaselinePieces(store, baselineTuningCtx.tuning, store.primaryType);
  const overhead = resolveOverheadHours(store, tuning.overheadScale);
  const crewSpeeds = crew
    .map((id) => effectiveEmployeeSpeedForRoles(state.employees.get(id), store.account, id, roles, modes))
  ...
  const residualAdjustment = resolveResidualAdjustmentForStore(store, crewSize, roles.supervisor);
  const overlap = getLastCrewOverlapRate(storeKey, crew);
  const lastResidual = state.lastDurationResidualByStore.get(storeKey);
  const lastCrewBias = overlap > 0 && Number.isFinite(lastResidual?.durationResidual) ? overlap * 0.15 * safeNumber(lastResidual.durationResidual) : 0;
  const onSiteDuration = Math.max(0, rawOnSiteDuration + residualAdjustment.biasHours + lastCrewBias);
  return { onSiteDuration, manHours: onSiteDuration * crewSize, crewSize };
}

function effectiveEmployeeSpeedForRoles(employee, account, employeeId, roles, modes) {
  const baseSpeed = displayEmployeeSpeed(employee, account);
  if (!employee) return baseSpeed;
  let factor = 1;
  if (roles.supervisor === employeeId) factor = Math.min(factor, getContributionFactor(modes.supervisor));
  if ((roles.rx || []).includes(employeeId)) factor = Math.min(factor, getContributionFactor(modes.rx));
  if ((roles.training || []).includes(employeeId)) factor = Math.min(factor, getContributionFactor(modes.training));
  if ((roles.earlyLate || []).includes(employeeId)) factor = Math.min(factor, getContributionFactor(modes.earlyLate));
  return baseSpeed * factor;
}

function getTuningForStore(store) {
  if (!store) return { tuning: state.modelTuning, scope: "global", label: "Global tuned parameters" };
  const segmentKey = state.storeSegmentByStoreKey.get(store.storeKey)?.segmentKey || `${store.account}||S1`;
  if (state.modelTuningByAccountSegment.has(segmentKey)) return { tuning: state.modelTuningByAccountSegment.get(segmentKey), scope: "account_segment", label: segmentKey };
  const typeKey = `${store.account}||${store.primaryType || "Unknown"}`;
  if (state.modelTuningByAccountType.has(typeKey)) return { tuning: state.modelTuningByAccountType.get(typeKey), scope: "account_type", label: typeKey };
  if (state.modelTuningByAccount.has(store.account)) return { tuning: state.modelTuningByAccount.get(store.account), scope: "account", label: store.account };
  return { tuning: state.modelTuning, scope: "global", label: "Global tuned parameters" };
}

function getBaselineTuningForStore(store) {
  if (!store) return { tuning: state.baselineTuning, scope: "global", label: "Global baseline tuning" };
  const segmentKey = state.storeSegmentByStoreKey.get(store.storeKey)?.segmentKey || `${store.account}||S1`;
  if (state.baselineTuningByAccountSegment.has(segmentKey)) return { tuning: state.baselineTuningByAccountSegment.get(segmentKey), scope: "account_segment", label: segmentKey };
  const typeKey = `${store.account}||${store.primaryType || "Unknown"}`;
  if (state.baselineTuningByAccountType.has(typeKey)) return { tuning: state.baselineTuningByAccountType.get(typeKey), scope: "account_type", label: typeKey };
  if (state.baselineTuningByAccount.has(store.account)) return { tuning: state.baselineTuningByAccount.get(store.account), scope: "account", label: store.account };
  return { tuning: state.baselineTuning, scope: "global", label: "Global baseline tuning" };
}

function resolveBaselinePieces(store, baselineTuning = state.baselineTuning, typeOverride = null) {
  ...
  const context = resolveContextBaseline(segmentStats, typeStats, officeStats, accountStats, contextMode, baselineTuning);
  if (storeCandidate > 0 && context.value > 0) {
    ...
    const minStoreWeight = Math.max(0, Math.min(0.9, safeNumber(baselineTuning?.minStoreWeight) || 0.45));
    const storeWeight = n > 0 ? Math.max(rawStoreWeight, minStoreWeight) : rawStoreWeight;
    return { value: storeWeight * storeCandidate + (1 - storeWeight) * context.value, source: `store ${storeMode} + ${context.source}`, modeLabel: `store=${storeMode}, context=${contextMode}`, blendLabel: `store weight ${fmt(storeWeight, 2)} (raw ${fmt(rawStoreWeight, 2)})` };
  }
  if (storeCandidate > 0) return { value: storeCandidate, source: `store ${storeMode}`, modeLabel: `store=${storeMode}, context=${contextMode}`, blendLabel: "store-only" };
  return { value: context.value, source: context.source, modeLabel: `store=${storeMode}, context=${contextMode}`, blendLabel: "context-only" };
}

function pickBaselineCandidate(stats, mode = "median") {
  if (!stats) return 0;
  if (mode === "trimmed" && stats.trimmedMeanPieces > 0) return stats.trimmedMeanPieces;
  if (mode === "recent" && stats.recentWeightedPieces > 0) return stats.recentWeightedPieces;
...
function resolveContextBaseline(segmentStats, typeStats, officeStats, accountStats, contextMode, baselineTuning) {
  const globalStats = { avgPieces: state.global.avgPieces, medianPieces: state.global.medianPieces, trimmedMeanPieces: state.global.trimmedMeanPieces, recentWeightedPieces: state.global.recentWeightedPieces };
  const candidates = [
    { key: "account segment", value: pickBaselineCandidate(segmentStats, contextMode), weight: safeNumber(baselineTuning?.segmentWeight) },
    { key: "account type", value: pickBaselineCandidate(typeStats, contextMode), weight: safeNumber(baselineTuning?.typeWeight) },
    { key: "account office", value: pickBaselineCandidate(officeStats, contextMode), weight: safeNumber(baselineTuning?.officeWeight) },
    { key: "account", value: pickBaselineCandidate(accountStats, contextMode), weight: safeNumber(baselineTuning?.accountWeight) },
    { key: "global", value: pickBaselineCandidate(globalStats, contextMode), weight: safeNumber(baselineTuning?.globalWeight) },
  ].filter((candidate) => candidate.value > 0 && candidate.weight > 0);
  if (!candidates.length) return { value: pickBaselineCandidate(globalStats, contextMode), source: "global" };
  const sumW = candidates.reduce((sum, candidate) => sum + candidate.weight, 0);
  const value = candidates.reduce((sum, candidate) => sum + candidate.value * candidate.weight, 0) / sumW;
  return { value, source: candidates.map((candidate) => candidate.key).join(" + ") };
}

function resolveOverheadHours(store, overheadScale = state.modelTuning.overheadScale) {
  if (store?.overheadBaseP20 > 0) return { value: Math.max(0, store.overheadBaseP20 * overheadScale), source: "store overhead" };
  const segmentKey = state.storeSegmentByStoreKey.get(store.storeKey)?.segmentKey || `${store.account}||S1`;
  const segmentStats = state.accountSegmentStats.get(segmentKey);
  if (segmentStats?.overheadBaseP20 > 0) return { value: Math.max(0, segmentStats.overheadBaseP20 * overheadScale), source: "account segment overhead" };
  const typeKey = `${store.account}||${store.primaryType || "Unknown"}`;
  const typeStats = state.accountTypeStats.get(typeKey);
  if (typeStats?.overheadBaseP20 > 0) return { value: Math.max(0, typeStats.overheadBaseP20 * overheadScale), source: "account type overhead" };
  const officeKey = `${store.account}||${store.officeName || "Unknown"}`;
  const officeStats = state.accountOfficeStats.get(officeKey);
  if (officeStats?.overheadBaseP20 > 0) return { value: Math.max(0, officeStats.overheadBaseP20 * overheadScale), source: "account office overhead" };
  const accountStats = state.accountGlobalStats.get(store.account);
  if (accountStats?.overheadBaseP20 > 0) return { value: Math.max(0, accountStats.overheadBaseP20 * overheadScale), source: "account overhead" };
  return { value: Math.max(0, safeNumber(state.global.overheadBaseP20) * overheadScale), source: "global overhead" };
}

function resolveResidualAdjustmentForStore(store, crewSize, supervisorId = "") {
  return resolveScopedResidualAdjustment({ store, crewSize, supervisorId, globalStats: state.residualGlobal, byStore: state.residualByStore, bySegment: state.residualByAccountSegment, byType: state.residualByAccountType, byOffice: state.residualByAccountOffice, byAccount: state.residualByAccount, byStoreCrewBand: state.residualByStoreCrewBand, byStoreSupervisor: state.residualByStoreSupervisor, byAccountSupervisor: state.residualByAccountSupervisor, bySegmentCrewBand: state.residualByAccountSegmentCrewBand, byTypeCrewBand: state.residualByAccountTypeCrewBand, byAccountCrewBand: state.residualByAccountCrewBand, byGlobalCrewBand: state.residualGlobalCrewBand, shrinkageKs: state.residualShrinkageKs, profilesByDensity: state.residualProfilesByDensity });
}

function resolveScopedResidualAdjustment(config) {
  const store = config.store;
  const crewBand = getCrewBand(crewSizeOrDefault(config.crewSize));
  const globalStats = config.globalStats || { count: 0, mean: 0, p10: 0, p25: 0, p75: 0, p90: 0, stdDev: 0 };
  const segmentKey = state.storeSegmentByStoreKey.get(store.storeKey)?.segmentKey || `${store.account}||S1`;
  const typeKey = `${store.account}||${store.primaryType || "Unknown"}`;
  const officeKey = `${store.account}||${store.officeName || "Unknown"}`;
  ...
  const profile = config.profilesByDensity?.[profileDensity] || config.profilesByDensity?.dense || null;
  const scoped = {
    ...
    supervisor: supervisorKey && config.byStoreSupervisor ? config.byStoreSupervisor.get(`${storeKey}||${supervisorKey}`) : null,
    accountSupervisor: supervisorKey && config.byAccountSupervisor ? config.byAccountSupervisor.get(`${accountKey}||${supervisorKey}`) : null,
  ...
  const kOf = (key, fallback) => Number.isFinite(profileKs[key]) ? profileKs[key] : Number.isFinite(ks[key]) ? ks[key] : fallback;
  const minOf = (key, fallback) => Number.isFinite(profileMins[key]) ? profileMins[key] : fallback;
  const floorOf = (key, fallback = 0) => Number.isFinite(profile?.floors?.[key]) ? profile.floors[key] : fallback;
  ...
  biasHours = blendFromScope(scoped.store, biasHours, kOf("store", 8), floorOf("minStoreWeight", 0.3), (scoped.store?.count || 0) <= 2 ? 0.6 : Number.POSITIVE_INFINITY);
  biasHours = blendFromScope(scoped.accountSupervisor, biasHours, kOf("accountSupervisor", 10));
  biasHours = blendFromScope(scoped.supervisor, biasHours, kOf("storeSupervisor", 6));
  ...
  const anchorShare = Math.max(0, Math.min(1, floorOf("anchorMinShare", profileDensity === "sparse" ? 0.55 : 0.4)));
  const anchorWeightFloor = Math.max(0, Math.min(1, floorOf("anchorMinWeight", profileDensity === "sparse" ? 0.45 : 0.35)));
  const anchorK = Math.max(1, floorOf("anchorK", profileDensity === "sparse" ? 6 : 8));
  const anchorMaxAbs = Math.max(0, floorOf("anchorMaxAbsHours", profileDensity === "sparse" ? 0.9 : 0.7));
  if (storeCount > 0 && Math.abs(storeMean) > 0) {
    const desired = Math.max(-anchorMaxAbs, Math.min(anchorMaxAbs, storeMean * anchorShare));
    const needsAnchor = Math.sign(biasHours) !== Math.sign(desired) || Math.abs(biasHours) < Math.abs(desired);
    if (needsAnchor) {
      const anchorWeight = Math.max(anchorWeightFloor, storeCount / (storeCount + anchorK));
      biasHours = biasHours + anchorWeight * (desired - biasHours);
    }
  }
  const rangeCandidates = [
    { scope: "store+crew-band", stats: scoped.storeBand, min: minOf("storeBand", 5) },
    { scope: "store+supervisor", stats: scoped.supervisor, min: minOf("storeSupervisor", 6) },
    { scope: "store", stats: scoped.store, min: minOf("store", 8) },
    { scope: "account+supervisor", stats: scoped.accountSupervisor, min: minOf("accountSupervisor", 10) },
    { scope: "account+segment+crew-band", stats: scoped.segmentBand, min: minOf("segmentBand", 10) },
    { scope: "account+segment", stats: scoped.segment, min: minOf("segment", 16) },
    { scope: "account+type+crew-band", stats: scoped.typeBand, min: minOf("typeBand", 12) },
    { scope: "account+type", stats: scoped.type, min: minOf("type", 18) },
    { scope: "account+office", stats: scoped.office, min: minOf("office", 16) },
    { scope: "account+crew-band", stats: scoped.accountBand, min: minOf("accountBand", 16) },
    { scope: "account", stats: scoped.account, min: minOf("account", 24) },
    { scope: "global+crew-band", stats: scoped.globalBand, min: minOf("globalBand", 20) },
  ];
  let chosen = { scope: "global", stats: globalStats };
  rangeCandidates.some((item) => {
    if ((item.stats?.count || 0) >= item.min) { chosen = item; return true; }
    return false;
  });
  return { biasHours, lowOffset: safeNumber(chosen.stats?.p25), highOffset: safeNumber(chosen.stats?.p75), rangeScope: chosen.scope, rangeCount: safeNumber(chosen.stats?.count), profileDensity };
}

function blendFromScope(scopeStats, prior, k, minWeight = 0, maxDeltaAbs = Number.POSITIVE_INFINITY) {
  ...
  const limitedDelta = Number.isFinite(maxDeltaAbs) ? Math.max(-Math.abs(maxDeltaAbs), Math.min(Math.abs(maxDeltaAbs), delta)) : delta;
...
function inferSpeed(row) {
  ...
  if (row.totalExtQty > 0 && row.manHours > 0) return row.totalExtQty / row.manHours;
  return 0;
}
...
function chooseRobustBaseline(stats) {
  ...
  if (stats.jobCount >= 4 && stats.trimmedMeanPieces > 0) return stats.trimmedMeanPieces;
  if (stats.jobCount >= 3 && stats.recentWeightedPieces > 0) return stats.recentWeightedPieces;
  if (stats.medianPieces > 0) return stats.medianPieces;
  if (stats.trimmedMeanPieces > 0) return stats.trimmedMeanPieces;
  return stats.avgPieces > 0 ? stats.avgPieces : 0;
}
...
function summarizeJobGroup(jobGroup) {
  ...
  return {
    ...
    recentWeightedPieces: recentWeightedAverage(jobs.map((job) => ({ date: job.date, value: safeNumber(job.totalPieces) })), 5),
    avgDuration: mean(durations),
    medianDuration: median(durations),
    avgManHours: mean(manHours),
    durationStdDev: stdDev(trimmedDurations.length >= 2 ? trimmedDurations : durations),
...
function trimExtremes(values, pct = 0.05) {
  const cleanValues = [...(values || [])].map(safeNumber).filter(Number.isFinite).sort((a, b) => a - b);
...
function robustSpread(values) {
  const cleanValues = [...(values || [])].map(safeNumber).filter(Number.isFinite);
...
function applyAnalyticsSnapshot(snapshot) {
  ...
  state.baselineTuningByAccountSegment = entriesToMap(snapshot.baselineTuningByAccountSegment);
  ...
  state.residualByAccountSegmentCrewBand = entriesToMap(snapshot.residualByAccountSegmentCrewBand);
  state.residualByAccountTypeCrewBand = entriesToMap(snapshot.residualByAccountTypeCrewBand);
...
function mean(values) {
  ...
  return cleanVals.length ? cleanVals.reduce((sum, value) => sum + value, 0) / cleanVals.length : 0;
}

function median(values) {
  const cleanVals = (values || []).map(num).filter(Number.isFinite).sort((a, b) => a - b);
  if (!cleanVals.length) return 0;
  const mid = Math.floor(cleanVals.length / 2);
  return cleanVals.length % 2 ? cleanVals[mid] : (cleanVals[mid - 1] + cleanVals[mid]) / 2;
}
...
function percentile(values, pct) {
  ...
  return low === high ? cleanVals[low] : cleanVals[low] + (cleanVals[high] - cleanVals[low]) * (rank - low);
}
...
function stdDev(values) {
  ...
  const variance = cleanVals.reduce((sum, value) => sum + (value - avg) ** 2, 0) / (cleanVals.length - 1);
...
function sched(value) {
  ...
  return !text || text === "0" || /^#error$/i.test(text) || /^false$/i.test(text) ? "" : text;
}
...
function matchesEmployeeQuery(employeeId, query, displayName) {
  ...
  const boundaryMatcher = new RegExp(`(^|[^a-z0-9])${escapeRegExp(normalizedQuery)}`);
...
function resolveBulkEmployeeSelection(rawText, candidateIds) {
  ...
  const pool = Array.from(candidateIds || []).map((id) => clean(id)).filter(Boolean);
  ...
  structuredEntries.forEach((entryObj) => {
    ...
    const exactIdMatch = pool.find((id) => id.toLowerCase() === normalizedLower);
    if (exactIdMatch) {
      markMatched(exactIdMatch);
      return;
    }

    const queryMatches = pool.filter((id) => matchesEmployeeQuery(id, normalizedLower, nameOf(id)));
    ...
    const firstNameCandidates = pool.filter((id) => getEmployeeNameParts(id).first === firstNameToken);
    ...
    const narrowed = firstNameCandidates.filter((id) => getEmployeeNameParts(id).last.startsWith(lastToken));
...
function formatBulkSelectionMessage(result, label = "employees") {
  ...
  if (result.ambiguousEntries.length) {
    fragments.push(`Ambiguous: ${summarizeBulkEntryList(result.ambiguousEntries)} (add last initial).`);
  }
...
function scheduleOffice(row) {
  ...
  const matched = officeFlags.find(([, enabled]) => enabled === true || String(enabled).toLowerCase() === "true");
...
function fmt(value, decimals = 2) {
  return num(value).toLocaleString(undefined, { minimumFractionDigits: decimals, maximumFractionDigits: decimals });
}
...
function weekday(dateString) {
  const date = new Date(`${dateString}T00:00:00`);
  return Number.isNaN(date.getTime()) ? dateString : date.toLocaleDateString(undefined, { weekday: "long" });
}
...
function setEmployeeBulkStatus(message, tone = "") {
  ...
  dom.employeeBulkStatus.classList.remove("meta-warning", "meta-success", "is-hidden");
  if (!message) {
...
function showAuthOverlay(message = "") {
  ...
  if (!overlay) {
    ...
    document.getElementById("authSignInBtn")?.addEventListener("click", () => auth?.signInWithPopup(googleProvider).catch((error) => {
