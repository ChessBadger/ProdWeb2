...
export interface FilterState {
    ...
    store: string;
    supervisor: string;
    timeframe: 'all' | 'last7' | 'last30' | 'last180' | 'last365' | 'custom' | 'specific';
    startDate: string;
    endDate: string;
    specificDate: string;
    topN: number;
...
export const usePerformanceData = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const rawJsonData: { [key: string]: RawEmployeeRecord[] } = await response.json();
                
                const rawDataArray = Object.values(rawJsonData)[0] || [];

                if (!Array.isArray(rawDataArray)) {
                    throw new Error("Loaded data is not in the expected format (array not found).");
                }
                ...
                const transformedData: EmployeeRecord[] = rawDataArray.map(record => {
                    ...
                    const supervisorName = employeeIdToNameMap.get(supervisorId) || String(supervisorId);
    ...
    const uniqueValues = useMemo((): UniqueValues => {
        if (!data.length) {
            return { employees: [], accounts: [], offices: [], stores: [], supervisors: [] };
        }
